...
class Punkt:
    ...
    def x_koordinat(self, ny_verdi):
        if ny_verdi < 0:
            raise ValueError("Punktet kan ikke ha ett nagativt x-koordinat")
    ...
    # Gir en strengpresentasjon av objektet som skal gi mening for
    # en bruker
    def __str__(self):
        return f"Punkt: ({self.x_koordinat}, {self.y_koordinat})"
    
    # Gir eb stengpresentasjon til en mer internt bruk
    def __repr__(self):
        return f"Punkt: ({self.x_koordinat}, {self.y_koordinat})"
